        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.deduction.title_singular') }}
                </div>
                <div class="panel-body">

                    <form action="{{ route("admin.deductions.update", [$deduction->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group {{ $errors->has('year') ? 'has-error' : '' }}">
                            ...
                            <select id="year" name="year" class="form-control">
                                ...
                                @foreach(App\Deduction::YEAR_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('year', $deduction->year) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('year'))
                                <p class="help-block">
                        ...
                        <div class="form-group {{ $errors->has('month') ? 'has-error' : '' }}">
                            ...
                            <select id="month" name="month" class="form-control" required>
                                ...
                                @foreach(App\Deduction::MONTH_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('month', $deduction->month) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('month'))
                                <p class="help-block">
